...
describe('getting votes and selecting merchant', function () {
  ...
  describe.skip('S1) get Address for Delivery Search', function () {
    before('setup admin', function * () {
      ...
      res = yield helper.initiateFoodOrdering(admin)
    })
  ...
  describe('S4) If we DON’T already have diet of Team Member', function () {
    // setup
    prevMode = 'food'
    prevAction = 'admin.confirm'
    before(function * () {
      this.timeout(5000)
      yield helper.setPrevModeActionRoute(prevMode, prevAction)
    })

    describe("check if ENV's are correct", function () {
      ...
      // expect(confirmMessage.attachments[0].actions[0].text.toLowerCase()).to.eql('confirm')
      })
    })
    describe('presume admin pressed "confirm"', function () {
      // var res = yield admin.tap(confirmMessage, 0, 0)
