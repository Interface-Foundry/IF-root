...
export default class SearchCard extends Component {
  ...
  render() {
    const { thumbnail_url, image_url, name, price, cart_id, index, unit_type } = this.props;
    return (
      <Link to={`/cart/${cart_id}/m/search/${index}/${encodeURIComponent(getLastSearch())}`}>
        <section className='card__type-search'>
          <div className='details'>
            ...
            <div className='details__price'>{displayCost(price)}<span>{unit_type ? unit_type === 'PK' ? ' /package' : ' /each' : ''}</span></div>
          </div>
