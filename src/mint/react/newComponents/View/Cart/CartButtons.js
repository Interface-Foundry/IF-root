  ...
  render() {
    ...
    return (
      <div className='action'>
        { 
          !cart.locked && user.id && editId === item.id && user.id === item.added_by ? <div className='update'>
        ...
        { !cart.locked && user.id && editId === item.id && user.id === item.added_by ? <button onClick={() => removeItem(cart.id, item.id)}>Remove Item</button> : null }
        { !cart.locked && user.id && editId !== item.id && user.id === item.added_by ? <button onClick={() => editItem(item.id)}><span>Edit Item</span></button> : null }
