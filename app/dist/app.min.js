...
},e.loadLandmarks=function(){m.getLandmarks(e.world._id).then(function(a){F({landmarks:a}),E()})},e.$on("landmarkCategoryChange",function(a,b){function c(a){return a.category===b}var d=e.landmarks.filter(c).map(I);d.length>0&&(K.setCenterFromMarkers(d),K.setMarkers(d),e.aperture.set("full"),e.selectedCategory=b)}),m.getWorld(c.worldURL).then(function(a){e.loadWorld(a)},function(a){})}]);