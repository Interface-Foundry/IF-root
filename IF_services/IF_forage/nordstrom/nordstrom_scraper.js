    ...
    return new Promise(function(resolve, reject) {
        ...
        request(options, function(error, response, body) {
            if ((!error) && (response.statusCode == 200)) {
                ...
                //get the styleId to query nordstrom server with from the product URL. lastindexof gets item from end of URL. split('?') kills anything after productID in URL
    ...
    return new Promise(function(resolve, reject) {
        ...
        request(options, function(error, response, body) {
            if ((!error) && (response.statusCode == 200)) {
                ...
                // //list of stores that are currently carrying this item
                // for (var i = 0; i < body["PersonalizedLocationInfo"].Stores.length; i++) { 
                //     //physicalStores.push(body["PersonalizedLocationInfo"].Stores[i].StoreNumber);
                //     var storeNum = body["PersonalizedLocationInfo"].Stores[i].StoreNumber;
                //     setTimeout(function() { getStore(storeNum) }, 1000);   
