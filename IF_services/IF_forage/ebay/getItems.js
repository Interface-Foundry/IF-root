...
function run() {
    db.EbayCategories.find({}).sort({
        'CategoryLevel': -1
    }).exec(function(err, categories) {
        async.eachSeries(categories, function iterator(category, finishedCategory) {
                console.log('\n\n\nStarting Category: ', category.CategoryName, ' Level: ', category.CategoryLevel, '\n\n\n');
                var pageNum = 1;
                var pageMax;
                async.doWhilst(
    ...
    return new Promise(function(resolve, reject) {
        ...
        request(options, function(error, response, body) {
            if ((!error) && (response.statusCode == 200)) {
                ...
                newItem.descriptionTags = tagParser.parseRelevant(_.uniq(descTags.concat(specTags)))
                try {
