...
var stitch = require('../image_processing/api')
var banter = require('./banter')
var nlp = require('../nlp/api');
require('colors');

//
// Print debug messages with this command:
// DEBUG=chat node server.js
//
var debug = require('debug')('chat')

//
// inventory providers, so far just amazon
//
var amazon = require('./amazon');


//
// HTTP server serves the single page app
//
var indexhtml = fs.readFileSync(__dirname + '/index.html', 'utf8')
var createServerSnippet =  function(req, res) {
  debug('client requested index.html')
  res.send(indexhtml);
}
var app = http.createServer(createServerSnippet).listen(8000);
console.log('server listening on ' + 'port 8000'.rainbow);


//
// in-memory history of messages for each user
// (note that on restart we lose the messages)
// it's a hash of singly-linked lists, where each state
// contains a reference to the previous state
// each hash key is a unique user
//
var messageHistory = {};
...
io.on('connection', function(socket) {
    ...
    var sname = 'socket ' + s.id;
    debug('%s connected', sname)

    //
    // process messages
    //
    socket.on("msgToClient", function(data) {
        debug('%s received msgToClient: "%s"', sname, data.msg)

        // check for small talk
        var res = banter(data);
        if (res) {
            debug('responding to %s with message "%s"', sname, res.message)
            socket.emit("msgFromSever", res)
        }

        // use nlp to obtain features which are then converted to messages
        nlp.parse(data.msg, function(e, features) {
            if (e) {
                console.error('error parsing message ' + data.msg);
            }

            // the message features and the previous state determine the rest
            getActionAndState({
                features: features,
                state: messageHistory[socket.id]
            }, function(err, action, state) {
                if (err) {
                    console.error('error getting action for message ' + data.msg);
                }
                messageHistory[socket.id] = state; // TODO keep many states in history

                // emit one or more messages
                // TODO save to database
                if (action.message) {
                  socket.emit('msgFromSever', {message: action.message})
                } else if (action.messages) {
                  action.messages.map(function(m) {
                    socket.emit('msgFromSever', {message: m})
                  })
                }
            })
        });
...
function getActionAndState(data, callback) {
...
function saveToCart(data){
    ...
    recallHistory(data, function(item){
        ...
        async.eachSeries(data.searchSelect, function(searchSelect, callback) {
            ...
            outgoingResponse(data,'txt', socket);
        });
    });
}
    ...
    switch (type) {
        case 'initial':
            ...
            client.itemSearch(amazonParams).then(function(results){
              debug('got amazon results')
              //console.log('checking for amazon server error so we handle',results);

              outgoingResponse(results,'stitch', socket); //send back msg to user
                ...
                if (data.amazon){ //we have a previously saved amazon session
                    ...
                    client.similarityLookup({
                      ...
                      responseGroup: 'ItemAttributes,Offers,Images'
                        ...
                        //console.log('checking for amazon server error so we handle',results);
                        outgoingResponse(results,'stitch',socket); //send msg to user
                        saveHistory(data,results,'amazon'); //push new state, pass amazon results
...
function outgoingResponse(data, action, socket){ //what we're replying to user with
    debug('preparing response for action %s', action)
    //stitch images before send to user
    if (action == 'stitch'){
        stitchResults(data, 'amazon', function(url){
            console.log('TESTING FOR ERROR ',url);
            socket.emit("msgFromSever", {message: url});
        });
    }
    //single image msg to user
    else if (action == 'txt'){
        socket.emit("msgFromSever", {message: data.msg});
    }
    //single image msg to user
    ...
    //one default image msg to user
        ...
        socket.emit("msgFromSever", {message: data[0].LargeImage[0].URL[0]});
    }
}
