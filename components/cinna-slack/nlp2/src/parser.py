...
def parse(data):
    ...
    for sent in sentences:
        ...
        # item focus
        s.focus = []
        if (sent.words.count('first') > 0) or (sent.words.count('1') > 0) or (sent.words.count('one') > 0):
            s.focus.append(1)
        if (sent.words.count('second') > 0) or (sent.words.count('2') > 0) or (sent.words.count('two') > 0):
            s.focus.append(2)
        if (sent.words.count('third') > 0) or (sent.words.count('3') > 0) or (sent.words.count('three') > 0):
            s.focus.append(3)
